...
/**
 * Активировать таб по индексу (как клик по кнопке с data-tab-index).
 *
 * @param {HTMLElement} root
 * @param {number} tabIndex
 */
export function tabsActivateByIndex(root, tabIndex) {
  const btn = root.querySelector(`[data-tab-button][data-tab-index="${Number(tabIndex)}"]`)
  if (btn) {
    btn.click()
  }
}
 ...
 * @returns {number}
 */
export function tabsGetActiveIndex(root) {
  const active = root.querySelector('[data-tab-button][data-tab-active="true"]')
  if (!active) return 0
  const i = parseInt(active.getAttribute('data-tab-index') || '0', 10)
 ...
 * @returns {number}
 */
export function tabsButtonCount(root) {
  return root.querySelectorAll('[data-tab-button]').length
}
