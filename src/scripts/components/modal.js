...
export function openModal(modalId) {
  ...
  // Принудительный reflow + двойной RAF: браузер должен отрисовать начальное
  // состояние (opacity: 0, scale) до добавления класса open — иначе transition не запустится
...
export function closeModal(modalId) {
  ...
  document.dispatchEvent(new CustomEvent('knyaje:modal-closed'))
...
/** Есть ли сейчас открытая модалка, кроме указанной (для popup и т.п.) */
export function hasOtherModalsOpen(exceptModalId) {
  return getOpenModals().some((id) => id !== exceptModalId)
}

/**
 * Инициализация popup-модалки с отложенным показом и/или при уходе курсора.
 * @param {Object} config - Конфиг из WP (wp_localize_script) или data-атрибутов
 * @param {string} config.modalId - ID модалки (по умолчанию 'modal-popup')
 * @param {number} config.delaySeconds - Через сколько секунд показать (0 = не показывать по таймеру)
 * @param {boolean} config.onMouseLeave - Показывать при уходе курсора за пределы страницы
 * @param {boolean} config.oncePerSession - Показывать только раз за сессию (sessionStorage)
 */
export function initPopupModal(config = {}) {
  const modalId = config.modalId || 'modal-popup'
  const delaySeconds = parseInt(config.delaySeconds, 10) || 0
  const onMouseLeave = !!config.onMouseLeave
  const oncePerSession = config.oncePerSession !== false

  const modalElement = document.getElementById(modalId)
  if (!modalElement) return

  const storageKey = `knyaje-popup-shown-${modalId}`
  ...
  /** @returns {boolean} popup открыт или уже был открыт */
  function show() {
    if (!canShow()) return false
    if (isModalOpen(modalId)) return true
    // Не перебиваем форму, меню, видео и т.д.
    if (hasOtherModalsOpen(modalId)) return false
    if (oncePerSession) {
  ...
  let timeoutId = null
  let popupShowDeferred = false

  function tryFinishDeferredPopup() {
    if (!popupShowDeferred) return
    const opened = show()
    if (opened) {
      popupShowDeferred = false
      return
    }
    if (!canShow()) {
      popupShowDeferred = false
  ...
  document.addEventListener('knyaje:modal-closed', tryFinishDeferredPopup)
